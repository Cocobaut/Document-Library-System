...
export function LoginPage({ onLogin }: { onLogin: (token: string, role: Role, username: string, userId: string, unitName?: string) => void }) {
    ...
    /*
     * Evaluates password strength based on three criteria:
     * length >= 8, contains uppercase letter, contains digit or special character.
     *
     * @param pw - Password string to evaluate
     * @returns Tuple of three booleans representing each strength criterion
     */
    const pwStrength = (pw: string) => [pw.length >= 8, /[A-Z]/.test(pw), /[0-9!@#$%]/.test(pw)];
    const strength = pwStrength(newPw);
    // Determine label and color based on how many criteria are met (1=Weak, 2=Fair, 3=Strong)
