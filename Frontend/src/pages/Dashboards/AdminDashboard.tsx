 ...
 *
 * Top-level container for the admin role that routes between
 * the three admin panels (Units, Users, Analytics) based on
 * the active sidebar navigation item.
 */
...
/**
 * Admin dashboard component that renders the correct management panel
 * based on the active navigation selection.
 *
 * @param user - Current admin user record
 * @param activeNav - Active sidebar navigation item ID (units, users, analytics)
 */
export function AdminDashboard({ user, activeNav: rawNav }: { user: UserRecord, activeNav: string }) {
    // Fallback: redirect "home" and "settings" to "units" since admin has no home page
