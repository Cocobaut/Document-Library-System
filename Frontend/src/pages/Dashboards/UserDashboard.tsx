 ...
 * User Dashboard
 *
 * Home page for regular users displaying document count KPIs
 * (My Documents, Shared With Me, Inherited Documents).
 * Routes to DocumentsTab and UploadTab based on sidebar navigation.
 */
...
/**
 * Dashboard component for regular users showing document count summaries.
 *
 * @param user - Current user record
 * @param activeNav - Active sidebar navigation item (home, documents, upload)
 * @param navKey - Incremented on each nav click to force data re-fetch even for same tab
 */
export function UserDashboard({ user, activeNav, navKey }: { user: UserRecord, activeNav: string, navKey?: number }) {
    ...
    useEffect(() => {
        if (activeNav === "home") {
            setLoading(true);
            // Fetch a small page just to get document count totals for the KPI cards
