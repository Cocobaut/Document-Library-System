 ...
 *
 * Admin panel for managing organizational units (departments).
 * Responsible for:
 * - Listing all units with member/document counts
 * - Displaying detailed unit info with member table on selection
 * - Creating new units with name and storage quota
 * - Editing unit name and quota
...
/*
 * Full CRUD management panel for organizational units.
 * Features a master-detail layout: unit list on the left, detail view on the right.
 */
export function UnitsPanel() {
     ...
     * @param type - Visual style (success, error, or info)
     */
    const showToast = (msg: string, type: "success" | "error" | "info" = "success") => setToast({ msg, type });

    /*
     * Fetches all unit statistics from the admin API and updates the unit list.
     */
    ...
    /*
     * Fetches detailed information for a specific unit including member list.
     *
     * @param unitId - The unit to load details for
     */
    ...
    /*
     * Toggles unit selection. Clicking the same unit deselects it;
     * clicking a different unit loads its details.
     *
     * @param unitId - The unit to select or deselect
     */
    ...
    /*
     * Creates a new unit via the admin API.
     * Converts the quota from GB (user input) to bytes (API format).
     */
    ...
    /**
     * Updates an existing unit's name and/or quota.
     * Refreshes both the unit list and the detail view if the edited unit is selected.
     */
    const handleUpdateUnit = async () => {
        ...
        try {
            ...
            await loadUnits();
            // Refresh detail view if the currently selected unit was edited
    ...
    /*
     * Deletes a unit and clears the detail view if it was selected.
     *
                                    ...
                                    <tbody>
                                        {detail.members.length === 0 ? (
                                            ...
                                            <tr key={idx} className="border-b border-slate-50 hover:bg-slate-50/80 transition-colors last:border-0">
                                                ...
                                                <td className="px-4 py-3.5">
                                                    {/* Role badge with color coding by role level */}
                                                ...
                                                <td className="px-4 py-3.5 min-w-[160px]">
                                                    {/* Convert raw byte values to GB for display in storage bar */}
