 ...
 * Analytics Panel
 *
 * Admin analytics dashboard displaying:
 * - Four KPI cards (Total Units, Users, Documents, Quota Used)
 * - Bar chart showing document count per department
 * - Pie chart showing storage quota distribution by unit
 *
 * All data is fetched from three analytics API endpoints in parallel.
 */
...
/*
 * Analytics panel component that fetches and displays system-wide statistics
 * with KPI cards, bar chart, and pie chart visualizations.
 */
export function AnalyticsPanel() {
    ...
    /**
     * Loads all analytics data by fetching overview, document stats, and unit quotas
     * concurrently. Transforms raw API responses into chart-ready data structures.
     */
    const loadAnalytics = async () => {
        setLoading(true);
        setFetchError("");
        try {
            // Fetch all three analytics endpoints in parallel for faster load
            ...
            // Transform document stats into bar chart format: truncate unit names to 10 chars
            setBarData(docStats.details_by_unit.map(d => ({
                unit: d.unit_name.substring(0, 10),
                docs: d.total_documents
            })));

            // Transform quota data into pie chart format: convert bytes to GB
